...
export default async function AdminProductsPage({
  ...
  let query = supabase
    .from('products')
    .select('id, name, slug, price_pence, compare_at_price_pence, is_active, stock_quantity, sku, images', { count: 'exact' })
  ...
  if (status === 'low_stock') query = query.lte('stock_quantity', 5).eq('is_active', true)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">{count ?? 0} products</p>
        </div>
        <Link
            ...
            {(products ?? []).map((product) => {
              ...
              return (
                <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`font-medium ${(product.stock_quantity ?? 0) <= 5 ? 'text-red-600' : 'text-gray-900'}`}>
                      {product.stock_quantity ?? 0}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div>
                      <span className="font-medium text-gray-900">{formatGBPFromPence(product.price_pence ?? 0)}</span>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${product.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'}`}>
